...
2. 이 폴더의 `업로드.bat` 을 더블클릭하고, 물어보면 저장소 주소를 붙여넣는다.
...
5. 아이패드 **Safari**로 그 주소를 연다. (크롬이 아님)
6. 공유 버튼 → **홈 화면에 추가** → 추가.
...
화면을 고친 뒤 `업로드.bat` 을 다시 실행하면 됩니다. 1~2분 뒤 아이패드에서 앱을 쓸어 올려 완전히 종료하고 다시 열면 새 버전이 뜹니다.

## 알아둘 점

- 품목·재고·PIN은 **기기마다 따로** 저장됩니다. 태블릿과 아이패드는 목록이 공유되지 않습니다.
- 확률은 등급끼리만 적용되고, 같은 등급 안에서는 남은 재고가 많을수록 자주 나옵니다.
- 뽑기 화면의 확률 고지는 설정에서 숫자를 바꾸는 즉시 반영됩니다.
- 직원 PIN 기본값: `1234`
